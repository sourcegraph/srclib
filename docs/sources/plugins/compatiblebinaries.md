# `srclib` compatibility

Editor plugins are temporarily incompatible with srclib `master`.

To use editor plugins, please download one of the following `srclib` binaries:
...
Once downloaded, unzip and place the `src` program in your `$PATH`.
