...
func TestPaths(t *testing.T) {
	cases := []struct {
		...
		{`func _() { if true { var a int; _ = a } }`, []defPath{{"foo", "_/$0/$0/a"}}, nil},
		{`type A int; func (a A) F() {}`, []defPath{{"foo", "A/F"}}, nil},
		{`type A int; func (a *A) F() {}`, []defPath{{"foo", "A/F"}}, nil},
		{`func F() {f := func(a int) (b int) { c := 7; return c; }; _ = f }`, []defPath{{"foo", "F/f"}, {"foo", "F/$0/a"}, {"foo", "F/$0/b"}, {"foo", "F/$0/c"}}, nil},
		{`func F() { {a:=0;_=a};{a:=0;_=a} }`, []defPath{{"foo", "F/$0/a"}, {"foo", "F/$1/a"}}, nil},
		{`func init() {}; func init() {}`, []defPath{{"foo", "init$28"}, {"foo", "init$44"}}, nil},
		...
		{`type f func(i int); type g func(i int)`, []defPath{{"foo", "$sources[0]/$0/i"}, {"foo", "$sources[0]/$1/i"}}, nil},
	}
